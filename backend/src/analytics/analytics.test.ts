...
const event = {
  ...
  path: "/blog/dating-without-swiping",
...
test("analytics rejects identifiers, arbitrary paths, source text and events", () => {
  ...
  for (const extra of [
    ...
    assert.equal(
      analyticsInput.safeParse({ ...event, ...extra }).success,
      false,
    );
  }
});
...
test("HTTP collection validates origin and payload and writes only a daily aggregate", async (t) => {
  ...
  t.mock.method(DailyMetric, "updateOne", (...args: unknown[]) => {
    writes.push(args);
    return Promise.resolve({ acknowledged: true });
  });
  const post = (payload: unknown, origin = "https://www.rosemarry.app") =>
    fetch(url, {
      method: "POST",
  ...
  assert.equal((await post(event, "http://localhost:4200")).status, 403);
  assert.equal(
    (await post({ ...event, email: "private@example.com" })).status,
    400,
  );
  assert.equal(writes.length, 0);
  assert.equal((await post(event)).status, 204);
  assert.equal(writes.length, 1);
  const update = writes[0]?.[1] as {
    $inc: { count: number };
