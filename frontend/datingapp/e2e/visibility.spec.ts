...
test('journal navigation updates metadata and removes article tags on the homepage', async ({ page }) => {
  const errors: string[] = [];
  ...
  await expect(page.locator('link[rel="canonical"]')).toHaveAttribute('href', 'https://www.rosemarry.app/blog/attraction-over-time');
...
test('article is readable on mobile and with JavaScript disabled', async ({ browser }) => {
  ...
  await expect(page.getByRole('heading', { name: 'What choice-overload research found' })).toBeVisible();
...
for (const successful of [true, false]) {
  test(`signup ${successful ? 'success records one conversion' : 'failure records no conversion'}`, async ({ page }) => {
    ...
    await page.route('https://rosemarry-api.onrender.com/**', async (route) => {
      const headers = { 'access-control-allow-origin': 'https://www.rosemarry.app', 'access-control-allow-headers': 'content-type', 'access-control-allow-methods': 'POST,OPTIONS' };
      ...
      if (route.request().url().endsWith('/analytics')) {
        ...
        registrations++;
        await route.fulfill({ status: successful ? 201 : 503, headers, contentType: 'application/json', body: JSON.stringify({ success: successful, message: successful ? 'Registered' : 'Temporarily unavailable' }) });
      }
    });
    await page.goto('https://www.rosemarry.app/blog/dating-without-swiping?utm_source=instagram&email=private@example.com');
    ...
    if (successful) {
      await expect(page.locator('.footer-signup__success')).toContainText('We saved reader@example.com');
      await expect.poll(() => events.filter((event) => event['event'] === 'signup_success').length).toBe(1);
    } else {
    ...
    expect(events.find((event) => event['event'] === 'signup_start')).toMatchObject({ source: 'social', placement: 'article', landing: '/blog/dating-without-swiping' });
